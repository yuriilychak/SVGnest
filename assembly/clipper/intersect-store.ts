...
export default class IntersectStore {
  ...
  private _buildIntersectList(botY: f64, topY: f64): void {
    if (this._activeEdge.isEmpty) {
      return;
    }

    let edge: TEdge = this._sortedEdge.update(
    ...
    while (isModified && !this._sortedEdge.isEmpty) {
      isModified = false;
      edge = this._sortedEdge.source as TEdge;
  ...
  private _intersectEdges(
    ...
    if (edge1.polyType == edge2.polyType) {
      if (edge1.isEvenOddFillType(this._subjFillType, this._clipFillType)) {
        const oldE1WindCnt: f64 = edge1.windCount1;
    ...
    const e1Wc: f64 = edge1.alignWindCount(
      this._subjFillType,
      this._clipFillType,
      false
    );
    const e2Wc: f64 = edge2.alignWindCount(
      this._subjFillType,
      this._clipFillType,
      false
    );
    ...
    if (e1Contributing && e2Contributing) {
      ...
      if (edge1.polyType != edge2.polyType) {
        ...
        const e1Wc2: f64 = edge1.alignWindCount(
          this._clipFillType,
          this._subjFillType,
          true
        );
        const e2Wc2: f64 = edge2.alignWindCount(
          this._clipFillType,
          this._subjFillType,
          true
        );
  ...
  private _processIntersectList(): void {
    const intersectionCount: u16 = u16(this._intersections.length);
    let i: u16 = 0;
  ...
  private _fixupIntersectionOrder(): boolean {
    ...
    this._sortedEdge.update(this._activeEdge.source as TEdge);
  ...
  public processIntersections(botY: f64, topY: f64): boolean {
    if (this._activeEdge.isEmpty) {
      return true;
    }
  ...
  private _addLocalMinPoly(edge1: TEdge, edge2: TEdge, point: Point): OutPt {
    ...
    let result: OutPt = this._outPolygon.addOutPt(primaryEdge, point) as OutPt;
    ...
    if (
      ...
      const outPt: OutPt = this._outPolygon.addOutPt(prevEdge, point) as OutPt;
  ...
  private _insertLocalMinimaIntoAEL(
    ...
    if (edge2 !== null) {
      this._activeEdge.insert(edge2 as TEdge, edge1);
    }
  ...
  public insertLocalMinimaIntoAEL(leftBound: TEdge, rightBound: TEdge): void {
    ...
    const outPt1: OutPt = this._insertLocalMinimaIntoAEL(edge1, edge2) as OutPt;
    let outPt2: OutPt;
    ...
    if (leftBound.ael.next != rightBound) {
      ...
      let edge: TEdge = leftBound.ael.next as TEdge;

      if (edge !== null)
        while (edge != rightBound) {
          this._intersectEdges(rightBound, edge, leftBound, false);

          edge = edge.ael.next as TEdge;
        }
    }
  }

  public processEdgesAtTopOfScanbeam(topY: f64, strictlySimple: boolean): void {
    let edge: TEdge = this._activeEdge.source as TEdge;
    let outPt: OutPt | null = null;
    ...
    while (edge !== null) {
      ...
      if (isMaximaEdge) {
        maxPairEdge = edge.getMaximaPair() as TEdge;
        isMaximaEdge = maxPairEdge === null || !maxPairEdge.isHorizontalY;
      }

      if (isMaximaEdge) {
        prev = edge.ael.prev as TEdge;
        this._doMaxima(edge);
        edge = (
          prev === null ? this._activeEdge.source : prev.ael.next
        ) as TEdge;
      } else {
        ...
        if (strictlySimple) {
          ...
          if (
            ...
            const op2: OutPt = this._outPolygon.addOutPt(edge, edge) as OutPt;
            this._joinStore.add(outPt as OutPt, op2, edge);
          }
        }
        edge = edge.ael.next as TEdge;
      }
    ...
    edge = this._activeEdge.source as TEdge;
    ...
    while (edge !== null) {
      if (edge.isIntermediate(topY)) {
        ...
        if (edge.outIndex >= 0) {
          outPt = this._outPolygon.addOutPt(edge, edge.top) as OutPt;
        }
  ...
  private _processHorizontal(horzEdge: TEdge, isTopOfScanbeam: boolean): void {
    ...
    if (lastHorzEdge.nextInLML === null) {
      maxPairEdge = lastHorzEdge.getMaximaPair() as TEdge;
    }

    while (true) {
      isLastHorz = horzEdge == lastHorzEdge;
      edge = horzEdge.ael.getNext(dirData.direction) as TEdge;
      ...
      while (edge !== null) {
        ...
        nextEdge = edge.ael.getNext(dirData.direction) as TEdge;
        ...
        if (dirData.getIncluded(edge)) {
          ...
          if (!isProtected) {
            if (maxPairEdge !== null && maxPairEdge.outIndex >= 0) {
    ...
    if (horzEdge.nextInLML !== null) {
      if (horzEdge.outIndex >= 0) {
        const op1: OutPt = this._outPolygon.addOutPt(
          horzEdge,
          horzEdge.top
        ) as OutPt;
  ...
  private _updateJoins(edge: TEdge, outPt1: OutPt): void {
    const joinEdge: TEdge = edge.getJoinsEdge() as TEdge;

    if (joinEdge === null) {
      return;
    }

    const outPt2: OutPt = this._outPolygon.addOutPt(
      joinEdge,
      edge.bottom
    ) as OutPt;
  ...
  private _doMaxima(edge: TEdge): void {
    const maxPairEdge: TEdge = edge.getMaximaPair() as TEdge;
    ...
    let nextEdge: TEdge = edge.ael.next as TEdge;
    ...
    while (nextEdge !== null && nextEdge != maxPairEdge) {
      ...
      nextEdge = edge.ael.next as TEdge;
    }
