...
export default class ClipperOffset {
  ...
  private _delta: f64 = 0;
  private _sinA: f64 = 0;
  private _sin: f64 = 0;
  private _cos: f64 = 0;
  private _miterLim: f64 = 0;
  private _stepsPerRad: f64 = 0;
  private _lowest: Point = Point.empty();
  private _polyNodes: PolyNode = new PolyNode();
  private _miterLimit: f64 = 0;
  private _arcTolerance: f64 = 0;

  constructor(
    miterLimit: f64 = ClipperOffset.defArcTolerance,
    arcTolerance: f64 = 2
  ) {
    this._lowest.x = -1;
  ...
  public addPath(path: Point[], joinType: JoinType, endType: EndType): void {
    let lastIndex: i16 = i16(path.length - 1);
  ...
  public execute(solution: Point[][], delta: f64): void {
    solution.length = 0;
  ...
  private _fixOrientations(): void {
    const childCount: u16 = this._polyNodes.childCount;
    const lowestX: f64 = this._lowest.x;
    let node: PolyNode;
    let i: u16 = 0;
  ...
  private _doOffset(delta: f64): void {
    this._destPolys = [];
    this._delta = delta;
    ...
    const miterLimit: f64 = Math.max(this._miterLimit, 2);
    this._miterLim = 2 / (miterLimit * miterLimit);
    let y: f64 =
      this._arcTolerance <= 0
        ? ClipperOffset.defArcTolerance
        : Math.min(
            Math.abs(delta) * ClipperOffset.defArcTolerance,
            this._arcTolerance
          );
    //see offset_triginometry2.svg in the documentation folder ...
    const angle: f64 = Math.acos(1 - y / Math.abs(delta)) * 2;
    const steps: f64 = (Math.PI * 2) / angle;
    ...
    for (i = 0; i < this._polyNodes.childCount; ++i) {
      ...
      const len: u16 = u16(this._srcPoly.length);
      if (
        len == 0 ||
  ...
  private _offsetWithType(type: EndType, j: u16, k: u16): void {
  ...
  private _offsetPoint(j: u16, k: u16, joinType: JoinType): u16 {
    this._sinA = Math.max(
    ...
    if (this._sinA * this._delta < 0) {
      ...
      switch (joinType) {
        case JoinType.Miter: {
          const r: f64 = 1 + this._normals.at(k).dot(this._normals.at(j));
  ...
  private _doSquare(j: u16, k: u16): void {
    const dx: f64 = Math.tan(
  ...
  private _doRound(j: u16, k: u16): void {
    const a: f64 = Math.atan2(
      this._sinA,
      this._normals.at(k).dot(this._normals.at(j))
    );
    const steps: u32 = u32(clipperRound(this._stepsPerRad * Math.abs(a)));
  ...
  private _doMiter(j: u16, k: u16, r: f64): void {
    this._insert(
  ...
  private _insertSquare(normalIndex: u16, polyIndex: u16, scale: f64): void {
  ...
  private _insert(point: Point, polyIndex: u16, scale: u16): void {
    this._destPoly.push(
      Point.from(point)
        .scale(scale)
  ...
  static defArcTolerance: f64 = 0.25;
}
