...
export default class Clipper {
  ...
  public addPaths(
    ...
    const polygonCount: u16 = u16(ppg.length);
    let i: u16 = 0;
  ...
  public addPath(
    ...
    const lastIndex: i16 = Clipper._getPolygonLastIndex(polygon, isClosed);
    ...
    if (!isClosed && startEdge.prev !== null) {
    ...
    if (isFlat) {
      ...
      if (locMin.right !== null) {
        locMin.right.side = EdgeSide.Right;
        locMin.right.windDelta = 0;
      }

      while (edge1.next !== null && edge1.next.outIndex != TEdge.skip) {
        edge1.nextInLML = edge1.next;

        if (edge1.prev !== null && edge1.bottom.x !== edge1.prev.top.x) {
    ...
    while (true) {
      edge1 = edge1.nextLocMin as TEdge;
      ...
      isClockwise = edge1.prev !== null && edge1.deltaX >= edge1.prev.deltaX;
      ...
      edge1 = this._localMinimaStore.processBound(
        localMinima.left as TEdge,
        isClockwise
      ) as TEdge;
      edge2 = this._localMinimaStore.processBound(
        localMinima.right as TEdge,
        !isClockwise
      ) as TEdge;
  ...
  private _insertLocalMinimaIntoAEL(botY: f64): void {
    let leftBound: TEdge;
    ...
    while (
      ...
      leftBound = this._localMinimaStore.current.left as TEdge;
      rightBound = this._localMinimaStore.current.right as TEdge;
  ...
  private _execute(): boolean {
    try {
      ...
      let botY: f64 = this._scanbeamStore.pop();
      let topY: f64;
  ...
  public static cleanPolygon(polygon: Point[], distance: f64 = 1.415): Point[] {
    let pointCount: u16 = u16(polygon.length);
    ...
    const squareDistance: f64 = distance * distance;
    let i: u16 = 0;
    let outPt: OutPt;
    ...
    while (outPt.index == 0 && outPt.next !== outPt.prev) {
      if (Point.pointsAreClose(outPt, outPt.prev as Point, squareDistance)) {
  ...
  public static area(polygons: Point[]): f64 {
  ...
  public static cleanPolygons(polys: Point[][], distance: f64): Point[][] {
    const result: Point[][] = new Array(polys.length);
    const polygonCount: u16 = u16(polys.length);
    let i: u16 = 0;
  ...
  private static _getPolygonLastIndex(
    ...
    let result: i16 = polygon.length - 1;
