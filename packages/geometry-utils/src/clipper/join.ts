...
export default class Join {
    ...
    public joinPoints(isRecordsSame: boolean, isUseFullRange: boolean): boolean {
        ...
        if (isHorizontal && this.OffPt.almostEqual(this.OutPt1.point) && this.OffPt.almostEqual(this.OutPt2.point)) {
            ...
            const value: PointF64 = Join.getOverlap(op1.point.x, op1b.point.x, op2.point.x, op2b.point.x);
            ...
            const reverse1: boolean =
                op1b.point.y > op1.point.y || !PointF64.slopesEqual(op1.point, op1b.point, this.OffPt, isUseFullRange);
            ...
            if (reverse1) {
                ...
                if (op1b.point.y > op1.point.y || !PointF64.slopesEqual(op1.point, op1b.point, this.OffPt, isUseFullRange)) {
                    return false;
                }
            }
            ...
            const reverse2: boolean =
                op2b.point.y > op2.point.y || !PointF64.slopesEqual(op2.point, op2b.point, this.OffPt, isUseFullRange);
            ...
            if (reverse2) {
                ...
                if (op2b.point.y > op2.point.y || !PointF64.slopesEqual(op2.point, op2b.point, this.OffPt, isUseFullRange)) {
                    return false;
                }
            }
    ...
    public static getOverlap(a1: number, a2: number, b1: number, b2: number): PointF64 {
        if (a1 < a2) {
            return b1 < b2
                ? PointF64.create(Math.max(a1, b1), Math.min(a2, b2))
                : PointF64.create(Math.max(a1, b2), Math.min(a2, b1));
        }

        return b1 < b2 ? PointF64.create(Math.max(a2, b1), Math.min(a1, b2)) : PointF64.create(Math.max(a2, b2), Math.min(a1, b1));
    }
}
